...
class BasicVSRNet(nn.Module):
    ...
    def __init__(self, scale=4, mid_channels=64, num_blocks=30, spynet_pretrained=None, upsampler='pixelshuffle'):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            if self.scale == 2:
                self.conv_up1 = nn.Conv2d(mid_channels, 64, 3, 1, 1)
            elif self.scale == 4:
                self.conv_up1 = nn.Conv2d(mid_channels, mid_channels, 3, 1, 1)
                self.conv_up2 = nn.Conv2d(mid_channels, 64, 3, 1, 1)
            self.conv_hr = nn.Conv2d(64, 64, 3, 1, 1)
            self.conv_last = nn.Conv2d(64, 3, 3, 1, 1)
    ...
    def forward(self, lrs):
        ...
        for i in range(0, t):
            ...
            if self.upsampler == 'pixelshuffle':
                if self.scale != 1:  # for denoise or blur
