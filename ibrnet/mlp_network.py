...
class IBRNet(nn.Module):
    def __init__(self, args, in_feat_ch=32, n_samples=64, **kwargs):
        ...
        # self.fc_q = nn.Sequential(nn.Linear(32+1+4, 32),
        #                             activation_func,
        #                             nn.Linear(32, 32))
        
        # self.fc_k = nn.Sequential(nn.Linear(32+1+4, 32),
        #                             activation_func,
        #                             nn.Linear(32, 32))
        
        # self.fc_v = nn.Sequential(nn.Linear(32+1+4, 32),
        #                             activation_func,
        #                             nn.Linear(32, 32))
        
        # self.fc_out = nn.Sequential(nn.Linear(32, 16),
        #                             activation_func,
        #                             nn.Linear(16, 8),
        #                             activation_func,
        #                             nn.Linear(8, 1))
        ...
        # if self.use_attn:
        #     self.fc_q.apply(weights_init)
        #     self.fc_k.apply(weights_init)
        #     self.fc_v.apply(weights_init)
    ...
    def forward(self, rgb_feat, ray_diff, mask):
        ...
        # import pdb
        # pdb.set_trace()
        if self.use_attn == False:
            x = torch.cat([x, vis, ray_diff], dim=-1)   #x:[416, 64, 6, 32], vis: [416, 64, 6, 1], ray_diff:[416, 64, 6, 4]
            x = self.rgb_fc(x)  #[416, 64, 6, 1]
        else:
            # idx = np.where(np.array(order_index) == min(order_index))
            # x_q = torch.cat([x[:, :, idx, :], vis[:, :, idx, :], ray_diff[:, :, idx, :]], dim=-1).unsqeeze(2) #[416, 64, 1, 37]
            # x_q = self.fc_q(x_q)    #[416, 64, 1, 32]
            x = torch.cat([x, vis, ray_diff], dim=-1)   #[416, 64, 6, 37]
            x_q = self.fc_q(x)      #[416, 64, 6, 32]
            x_k = self.fc_k(x)      #[416, 64, 6, 32]
            x_v = self.fc_v(x)      #[416, 64, 6, 32]
            attn_map = F.softmax((torch.einsum('nbik,nbjk->nbij', x_q, x_k)), dim=-1) #[416, 64, 6, 6]
            x_attn = torch.einsum('nbik,nbkj->nbij', attn_map, x_v)     #[416, 64, 6, 32]
            x = self.fc_out(x_attn)     #[416, 64, 6, 1]

        x = x.masked_fill(mask == 0, -1e9)
        blending_weights_valid = F.softmax(x, dim=2)  # color blending [416, 64, 6, 1]
        rgb_out = torch.sum(rgb_in*blending_weights_valid, dim=2)   #rgb_in:[416, 64, 6, 3]        
