...
if __name__ == '__main__':
    ...
    mean_coarse_psnr_list = []
    mean_fine_psnr_list = []
    mean_coarse_ssim_list = []
    mean_fine_ssim_list = []
    mean_coarse_lpips_list = []
    mean_fine_lpips_list = []

    print("eval_scenes", args.eval_scenes)
    ...
    for eval_scene in args.eval_scenes:
        print("eval_scene", eval_scene)
        scene_name = eval_scene
        # scene_name = args.eval_scenes[0]
        out_scene_dir = os.path.join(extra_out_dir, '{}_{:06d}'.format(scene_name, model.start_step))
        os.makedirs(out_scene_dir, exist_ok=True)

        # test_dataset = dataset_dict[args.eval_dataset](args, 'test', scenes=args.eval_scenes)
        test_dataset = dataset_dict[args.eval_dataset](args, 'test', scenes=eval_scene)
        print("test_dataset", test_dataset)
        save_prefix = scene_name
        test_loader = DataLoader(test_dataset, batch_size=1)
        print("test_loader", test_dataset)
        ...
        for i, data in enumerate(test_loader):
            ...
            with torch.no_grad():
                ...
                half_interval_length = 0.5  #TODO
                near_depth = (data['depth_range'] - half_interval_length).squeeze().cpu().numpy()   #TODO
                far_depth = (data['depth_range'] + half_interval_length).squeeze().cpu().numpy()    #TODO
                # coarse_pred_depth_colored = colorize_np(coarse_pred_depth, range=(near_depth, far_depth)) #TODO
        ...
        mean_coarse_psnr_list.append(mean_coarse_psnr)
        mean_fine_psnr_list.append(mean_fine_psnr)
        mean_coarse_ssim_list.append(mean_coarse_ssim)
        mean_fine_ssim_list.append(mean_fine_ssim)
        mean_coarse_lpips_list.append(mean_coarse_lpips)
        mean_fine_lpips_list.append(mean_fine_lpips)
        ...
        f.close()
    mean_coarse_psnr_all = sum(mean_coarse_psnr_list) / len(mean_coarse_psnr_list)
    mean_fine_psnr_all = sum(mean_fine_psnr_list) / len(mean_fine_psnr_list)
    mean_coarse_ssim_all = sum(mean_coarse_ssim_list) / len(mean_coarse_ssim_list)
    mean_fine_ssim_all = sum(mean_fine_ssim_list) / len(mean_fine_ssim_list)
    mean_coarse_lpips_all = sum(mean_coarse_lpips_list) / len(mean_coarse_lpips_list)
    mean_fine_lpips_all = sum(mean_fine_lpips_list) / len(mean_fine_lpips_list)
    f = open("{}/metrics_mean{}.txt".format(extra_out_dir, model.start_step), "w")
    f.write('mean_coarse_psnr_all: {} \n'
            'mean_fine_psnr_all: {} \n'
            'mean_coarse_ssim_all: {} \n'
            'mean_fine_ssim_all: {} \n'
            'mean_coarse_lpips_all: {} \n'
            'mean_fine_lpips_all: {} \n'
            .format( mean_coarse_psnr_all, mean_fine_psnr_all, mean_coarse_ssim_all,
                    mean_coarse_ssim, mean_coarse_lpips_all, mean_fine_lpips_all)
            )
    f.close()
